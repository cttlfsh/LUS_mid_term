...
with open("../data/data/NLSPARQL.train.data", "r") as f:
	...
	for line in f:
		if(line != "\n"):
			if (line.split("\t")[1][:-1] == 'O'):
				IOB_snt = IOB_snt + str(line.split("\t")[1][:-1])+"-"+str(line.split("\t")[0] + "\t")
				### Create a list of tuple (word, IOB)
	 			sentence.append([line.split("\t")[0], line.split("\t")[1][:-1]+"-"+line.split("\t")[0]])
	 			### Create a list of words
	 			words.append(line.split("\t")[0])
	 			IOB.append(line.split("\t")[1][:-1]+"-"+line.split("\t")[0])
	 		else:
...
if (threshold == 0):
	...
	### Generate all necessary files
	with open("w2IOB_without_O_cutoff/files/test_words_by_sentence.txt", "w") as f:
		for line in words_sentence:
			f.write(str(line) + "\n")
	with open("w2IOB_without_O_cutoff/files/train_IOB_by_sentence.txt", "w") as f:
		for line in IOB_sentence:
			f.write(str(line) + "\n")
	with open("w2IOB_without_O_cutoff/files/lexicon.txt", "w") as f:
		f.write("<eps>"+"\t"+"0"+"\n")
		for key, value in enumerate(lexicon):
			f.write(str(value+"\t"+str(key+1)+"\n"))
	with open("w2IOB_without_O_cutoff/files/automaton.txt", "w") as f:
		for element in automaton:
			f.write("0\t0\t"+element[0][0]+"\t"+element[0][1]+"\t"+element[1]+"\n")	
		f.write("0")
