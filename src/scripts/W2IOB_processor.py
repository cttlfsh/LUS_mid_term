...
if (threshold == 0):
	...
	with open("w2IOB_no_cutoff/files/test_words_by_sentence.txt", "w") as f:
		for line in words_sentence:
			f.write(str(line) + "\n")
	with open("w2IOB_no_cutoff/files/train_IOB_by_sentence.txt", "w") as f:
		for line in IOB_sentence:
			f.write(str(line) + "\n")
	with open("w2IOB_no_cutoff/files/lexicon.txt", "w") as f:
		f.write("<eps>"+"\t"+"0"+"\n")
		for key, value in enumerate(lexicon):
			f.write(str(value+"\t"+str(key+1)+"\n"))
	with open("w2IOB_no_cutoff/files/automaton.txt", "w") as f:
		for element in automaton:
